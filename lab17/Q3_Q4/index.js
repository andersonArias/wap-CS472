...
app.post("/addToCart", (req, res) => {
  sess = req.session;
  let prodSel = req.body.selPro;
  if (sess.products[prodSel.id]) {
    sess.products[prodSel.id].quantity++;
  } else {
    sess.products[prodSel.id] = prodSel;
  }
  console.log(
    "IN POST SESSION IS " +
      JSON.stringify(sess) +
      " IS " +
      JSON.stringify(sess.products)
  );
